...
static void swapHashTableValues(float* oldValues, float *newValues,
  ...
  CUDA_CHECK(
      cudaMemcpy(oldValues, table_values, size, cudaMemcpyDeviceToDevice));
  CUDA_CHECK(
      cudaMemcpy(table_values, newValues, size, cudaMemcpyDeviceToDevice));
...
void gpu_init(const float* features, HashTable* table, MatrixEntry* matrix,
  ...
  // on the CPU
  for (int i = 0; i < pd; i++) {
    scaleFactorHost[i] = (pd + 1) * sqrtf(
        (1.0 / 6 + blurVariance) / ((i + 1) * (i + 2)));
  }
  CUDA_CHECK(cudaMalloc((void**) &scaleFactor, sizeof(float) * pd));
  CUDA_CHECK(
      cudaMemcpy(scaleFactor, scaleFactorHost, sizeof(float) * pd,
...
void gpu_compute(Dtype* out, const Dtype* in, const HashTable &table,
  ...
  for (int color = reverse ? pd : 0; color <= pd && color >= 0; reverse ? color--
      : color++) {
...
void ModifiedPermutohedral::init_gpu(const float* features, int num_dimensions,
  ...
  if (!is_init) {
    ...
    CUDA_CHECK(
        cudaMalloc((void **) &matrix,
            sizeof(MatrixEntry) * (w * h * (num_dimensions + 1))));
  } else {
...
void ModifiedPermutohedral::compute_gpu(float* out, const float* in,
  ...
  switch (d_) {
  case 2:
    gpu_compute<2, float> (out, in, table, matrix, w_, h_, value_size, reverse,
        add);
    break;
  case 5:
    gpu_compute<5, float> (out, in, table, matrix, w_, h_, value_size, reverse,
        add);
    break;
  default:
    LOG(FATAL) << "num_dimensions should be 2 or 5";
...
void ModifiedPermutohedral::compute_gpu(double* out, const double* in,
  ...
  switch (d_) {
  case 2:
    gpu_compute<2, double> (out, in, table, matrix, w_, h_, value_size,
        reverse, add);
    break;
  case 5:
    gpu_compute<5, double> (out, in, table, matrix, w_, h_, value_size,
        reverse, add);
    break;
  default:
    LOG(FATAL) << "num_dimensions should be 2 or 5";
