...
#include "caffe/layers/fyu_image_label_data_layer.hpp"
...
template <typename Dtype>
FyuImageLabelDataLayer<Dtype>::FyuImageLabelDataLayer(
...
template <typename Dtype>
void FyuImageLabelDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
                                                const vector<Blob<Dtype>*>& top) {
  auto& data_param = this->layer_param_.fyu_image_label_data_param();
  ...
  const string& image_list_path = data_param.image_list_path();
  ...
  if (data_param.rand_skip()) {
    unsigned int skip = caffe_rng_rand() % data_param.rand_skip();
...
template <typename Dtype>
void FyuImageLabelDataLayer<Dtype>::ShuffleImages() {
//  caffe::rng_t* prefetch_rng =
//      static_cast<caffe::rng_t*>(prefetch_rng_->generator());
//  shuffle(lines_.begin(), lines_.end(), prefetch_rng);
//  LOG(FATAL) <<
//      "FyuImageLabelDataLayer<Dtype>::ShuffleImages() is not implemented";
  caffe::rng_t* prefetch_rng =
      static_cast<caffe::rng_t*>(prefetch_rng_->generator());
...
template <typename Dtype>
void FyuImageLabelDataLayer<Dtype>::SampleScale(cv::Mat *image, cv::Mat *label) {
  auto& data_param = this->layer_param_.fyu_image_label_data_param();
...
template <typename Dtype>
void FyuImageLabelDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
...
INSTANTIATE_CLASS(FyuImageLabelDataLayer);
REGISTER_LAYER_CLASS(FyuImageLabelData);
